...
目前, 我们可以通过 configuration yaml 文件方便的修改. 以支持DIV2K的 [PairedImageDataset](../fluxsr/data/paired_image_dataset.py) 为例, 根据不同的要求修改yaml文件:
...
实现是调用了[MMCV](https://github.com/open-mmlab/mmcv) 优雅的 FileClient 设计. 为了兼容 BasicSR, 我们对接口做了一些改动 (主要是为了适应LMDB), 参见 [file_client.py](../fluxsr/utils/file_client.py).

在实现我们自己的 dataloader 的时候, 可以方便地调用接口, 以实现对不同数据存储形式的支持, 具体可以参考 [PairedImageDataset](../fluxsr/data/paired_image_dataset.py) 的写法.
...
除了使用LMDB来加速外, 还可以采用预读取数据来加速, 实现参见 [prefetch_dataloader](../fluxsr/data/prefetch_dataloader.py).<br>
