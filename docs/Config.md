...
# network g settings
network_g:
  # Architecture type. Usually the class name defined in the `fluxsr/archs` folder
  type: MSRResNet
  #### The following arguments are flexible and can be obtained in the corresponding doc
...
train:
  ...
  pixel_opt:
    # Loss type. Usually the class name defined in the `fluxsr/models/losses` folder
    type: L1Loss
    # Loss weight
    loss_weight: 1.0
    # Loss reduction mode
...
val:
  ...
  metrics:
    ...
    psnr:
      # Metric type. Usually the function name defined in the`fluxsr/metrics` folder
      type: calculate_psnr
      #### The following arguments are flexible and can be obtained in the corresponding doc
...
logger:
  ...
  wandb:
    ...
    # Here, we use the fluxsr wandb project: https://app.wandb.ai/xintao/basicsr
    project: fluxsr
