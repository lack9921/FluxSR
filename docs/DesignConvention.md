...
Specifically, we implement it through `importlib` and `getattr`. Taking the data module as example, we follow the below steps in [`data/__init__.py`](../fluxsr/data/__init__.py):
...
_dataset_modules = [
    importlib.import_module(f'fluxsr.data.{file_name}')
...
| Model          | `_model.py`     | `fluxsr/models/sr_model.py` |
| Archs          | `_arch.py`      | `fluxsr/archs/srresnet_arch.py`|
...
| :------------- | :----------:    | :----------:   |
| Losses           | `fluxsr/models/losses`   | [`fluxsr/models/losses/__init__.py`](../fluxsr/models/losses/__init__.py) |
| Metrics          | `fluxsr/metrics`     | [`fluxsr/metrics/__init__.py`](../fluxsr/metrics/__init__.py)|
...
1. When logging, the loss items are recommended to start with `l_`, so that all these loss items will be grouped together in tensorboard. For example, in [basicsr/models/srgan_model.py](../fluxsr/models/srgan_model.py), we use `l_g_pix`, `l_g_percep`, `l_g_gan`, etc for loss items. In [fluxsr/utils/logger.py](../fluxsr/utils/logger.py), these items will be grouped together:
